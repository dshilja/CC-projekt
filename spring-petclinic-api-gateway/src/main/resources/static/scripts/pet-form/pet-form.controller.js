...
angular.module('petForm')
    .controller('PetFormController', ['$http', '$state', '$stateParams', function ($http, $state, $stateParams) {
        ...
        $http.get('api/customer/petTypes').then(function (resp) {
            ...
            if (petId) { // edit
                $http.get("api/customer/products/" + productId + "/pets/" + petId).then(function (resp) {
                    self.pet = resp.data;
        ...
        self.submit = function () {
            ...
            var req;
            if (id) {
                req = $http.put("api/customer/products/" + productId + "/pets/" + id, data);
            } else {
                req = $http.post("api/customer/products/" + productId + "/pets", data);
            }
